...
class GreedySolver:
    ...
    def _solve(self, pieces, current_loc):
        if len(pieces) == len(self.grid.pieces):
            self.solutions.append(pieces)
            return
        if current_loc[0] != 0:
            ...
            for p in solve_dict.keys():
                if p not in pieces.values():
                    ...
                    self._solve(pieces_cpy, (current_loc[0] - 1, current_loc[1]))
        else:
            ...
            for p in solve_dict.keys():
                if p not in pieces.values():
                    ...
                    self._solve(pieces_cpy, (current_loc[0], current_loc[1] - 1))

    def solve(self):
        self.solutions = []
        grid_size = self.grid.size
        for piece in self.grid.pieces:
            self._solve({(grid_size[0] - 1, grid_size[1] - 1): piece}, (grid_size[0] - 1, grid_size[1] - 1))
